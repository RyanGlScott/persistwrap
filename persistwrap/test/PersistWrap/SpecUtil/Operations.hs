...
genInsertion :: SchemaMap -> Int -> Gen (Text, Structure Text, Operation)
genInsertion m i = do
  (k, sv) <- elements $ Map.toList m
  withSomeSing sv $ \(s :: SStructure structure) -> do
    v :: EntityOf DummyFK structure <- withSingI s arbitrary
    return (k, fromSing s, Insert s i k v)
  ...
  arbitrary = do
    ...
    Operations sm <$> if xsLen == 0
      then return []
        ...
        (k, s0, i0) <- genInsertion sm 0
        rest     <- (`evalStateT` [(0, k, s0)]) $ forM [1 .. xsLen] $ \i -> mapStateTOneof
          [ do
            (name, s, res) <- lift (genInsertion sm i)
            State.modify ((i, name, s) :)
            return res
          , do
            insertionInds <- State.get
            (choice, name, s)   <- lift $ elements insertionInds
            return $ Lookup choice name s
          ]
        return $ i0 : rest
  shrink (Operations m ops) = do
    ...
    (do
      ...
      <|> (do
          ...
          ) <|> Operations m <$> subShrinkList opShrink ops

subShrinkList :: (a -> [a]) -> [a] -> [[a]]
subShrinkList subShrink xs = do
  (i, x) <- zip [0 ..] xs
  x'     <- subShrink x
...
looksUpIn xs = \case
  Insert{}     -> False
  Lookup i _ _ -> i `elem` map fst xs
...
data Operation where
  ...
  Lookup :: Int -> Text -> Structure Text -> Operation

opShrink :: Operation -> [Operation]
opShrink = \case
  Insert struct i name v -> Insert struct i name <$> withSingI struct shrink v
  Lookup{}               -> []
