...
spec :: Spec
spec = describe "Insertion and lookup" $ it "should behave like the model" $ property $ \ops ->
  ioProperty $ do
    ...
    go = (`evalStateT` IntMap.empty) $ forM ops $ \case
      Insert (singInstance -> SingInstance) i (toSing -> (SomeSing (sname :: SSymbol name))) x ->
        withSingI sname $ do
          ...
          return (i, Nothing)
      Lookup i (toSing -> SomeSing ((singInstance -> SingInstance) :: SStructure structure)) -> do
