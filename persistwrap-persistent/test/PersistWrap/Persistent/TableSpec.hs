...
module PersistWrap.Persistent.TableSpec
    ( spec
    ) where
...
import qualified PersistWrap.BackEnd.Persistent as BackEnd
import PersistWrap.Table.TH

import PersistWrap.Persistent.SpecUtil

sschemas :: Sing
  '[ $(schema "tab1" ["abc" ::: Nullable Int64])
   , $(schema "tab2" [])
   , $(schema "tab3" [ "hello" ::: Bool
...
spec = describe "Tables" $ it "should do row operations" $ do
  assertions :: Expectation <- withSystemTempDirectory "persistwrap-persistent-test" $ \tdir ->
    runStderrLoggingT
      $ BackEnd.withEmptyTableProxies (Text.pack $ tdir </> "dbfile") 100 sschemas
