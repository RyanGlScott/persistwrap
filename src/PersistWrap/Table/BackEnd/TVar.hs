...
import PersistWrap.Table.Class (entityKey, getTable)
...
instance Class.MonadTable Table STM where
  newtype Key Table tab = Key {unKey :: TVarMaybeRow (Schema tab)}
  ...
  insertRow :: forall tab . WithinTable tab => Row (Schema tab) -> STM (Key tab)
